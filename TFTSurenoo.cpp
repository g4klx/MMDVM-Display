...
void CTFTSurenoo::setIdleInt()
{
	...
	::snprintf(m_temp, sizeof(m_temp), "%s / %u", m_callsign.c_str(), m_id);
...
void CTFTSurenoo::writeDStarInt(const std::string& my1, const std::string& my2, const std::string& your, const std::string& type, const std::string& reflector)
{
	...
	::snprintf(m_temp, sizeof(m_temp), "%s %s/%s", type.c_str(), my1.c_str(), my2.c_str());
	setStatusLine(statusLineNo(0), m_temp);

	::snprintf(m_temp, sizeof(m_temp), "%s", your.c_str());
	setStatusLine(statusLineNo(1), m_temp);

	if (reflector != "        ")
		::snprintf(m_temp, sizeof(m_temp), "via %s", reflector.c_str());
	else
		::strcpy(m_temp, "");
...
void CTFTSurenoo::writeFusionInt(const std::string& source, const std::string& dest, unsigned char dgid, const std::string& type, const std::string& origin)
{
	setModeLine(STR_YSF);
	...
	if (origin != "          ")
		::snprintf(m_temp, sizeof(m_temp), "at %s", origin.c_str());
	else
		::strcpy(m_temp, "");
	setStatusLine(statusLineNo(2), m_temp);
...
void CTFTSurenoo::writeP25Int(const std::string& source, bool group, unsigned int dest, const std::string& type)
{
	setModeLine(STR_P25);
...
void CTFTSurenoo::writeNXDNInt(const std::string& source, bool group, unsigned int dest, const std::string& type)
{
	if (m_mode != MODE_NXDN)
...
void CTFTSurenoo::refreshDisplay()
{
	if (!m_refresh) return;
